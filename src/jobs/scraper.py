...
class JobScraper:
    ...
    async def _get_page_content(
        self, page: Page, url: str, wait_selector: str = "body"
    ) -> Optional[str]:
        try:
    ...
    async def scrape_naukri(self, role: str, location: str):
        ...
        async with async_playwright() as p:
            ...
            context = await browser.new_context(
                user_agent=self._get_random_user_agent(), viewport={"width": 1920, "height": 1080}
            )
            ...
            content = await self._get_page_content(
                page, url, "div.srp-jobtuple-wrapper, article.jobTuple, div.list"
            )
            if not content:
                await browser.close()
                return
            ...
            for job in jobs:
                try:
                    ...
                    company_el = job.find("a", {"class": "comp-name"}) or job.find(
                        "a", {"class": "subTitle"}
                    )

                    # Experience, Salary, Location are usually in a list
                    exp_el = (
                        job.find("span", {"class": "exp-wrap"})
                        or job.find("span", {"class": "exp"})
                        or job.find("li", {"class": "experience"})
                    )
                    sal_el = (
                        job.find("span", {"class": "sal-wrap"})
                        or job.find("span", {"class": "sal"})
                        or job.find("li", {"class": "salary"})
                    )
                    loc_el = (
                        job.find("span", {"class": "loc-wrap"})
                        or job.find("span", {"class": "loc"})
                        or job.find("li", {"class": "location"})
                    )

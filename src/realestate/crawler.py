...
class RealEstateCrawler:
    ...
    async def _get_page_content(self, page: Page, url: str) -> Optional[str]:
        try:
            ...
            await page.set_extra_http_headers({"User-Agent": self._get_random_user_agent()})
    ...
    async def scrape_99acres(self, city: str):
        ...
        async with async_playwright() as p:
            ...
            items = soup.find_all("div", {"class": "projectTuple"}) or soup.find_all(
            ...
            for item in items:
                try:
                    title_el = item.find("a", {"class": "srpTuple__propertyName"}) or item.find(
                        "a", {"class": "projectTuple__projectName"}
                    )
                    price_el = item.find("td", {"class": "srpTuple__price"}) or item.find(
                        "div", {"class": "list_header_semiBold"}
                    )
                    loc_el = item.find("a", {"class": "srpTuple__localityName"}) or item.find(
                        "div", {"class": "projectTuple__subHeadingWithLocality"}
                    )
